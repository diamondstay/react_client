    ...
    <Link to={'hotel/' + item.id}>
      <section className="promo-item">
        ...
        <div className="promo-content">
          ...
          <h4 className="promo-price">{formatPrice(item.price.data.nightly_price_vnd)}/đêm</h4>
          <h5 className="promo-location">{item.location}</h5>
          <div className="promo-ratings stars" dangerouslySetInnerHTML={{__html: Filter.getReviewStars(item.rating)}} />
        </div>
      </section>
    </Link>
  );
