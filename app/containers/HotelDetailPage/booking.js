      ...
      <section id="diamond-booking">
        <div className="booking-wrapper">
          <div className="booking-price">
            {
              promotion.percent > 0 ?
                <>
                  <span className="price">{Filter.formatVndCurrency(promotion.sale_price)}</span>
                  <span className="one-night">/đêm</span>
                  <span className="original-price strike">{Filter.formatVndCurrency(promotion.price)}</span>
                </> :
                <>
                  <span className="price">{Filter.formatVndCurrency(promotion.price)}</span>
                  <span className="one-night">/đêm</span>
                </>
            }
          </div>
          <Form className="book-form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <button className="btn book-btn" type="submit">Đặt ngay</button>
          </Form>
        </div>
        ...
        <div className="booking-wrapper">
          <div className="host-item">
            ...
            <div className="host-info">
              ...
              <h5 className="host-time">Tham gia {getJoinTime(host.created_at)}</h5>
            </div>
          </div>
        </div>
      </section>
